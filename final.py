...
if uploaded_file is not None:
    ...
    if multiple_emotions:
        left_column.write(f"<p style='font-size:16pt'>The emotions in the sound : {', '.join(multiple_emotions)} </p>", unsafe_allow_html=True)
    else:
        left_column.write(f"<p style='font-size:16pt'>The emotion in the sound : {final_emotion} </p>", unsafe_allow_html=True)
